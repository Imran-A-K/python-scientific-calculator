...
class Calc():
    ...
    def mod(self):
        self.result=False
        self.current=math.modf(math.radians(float(txtDisplay.get())))
        self.display(self.current)
...
btnSq=Button(calc, text="√",width=6, height=2, font=('arial',20,'bold'),bd=4, command=added_value.squared).grid(row=1, column=2, pady=1)
...
btnMult=Button(calc, text="*",width=6, height=2, font=('arial',20,'bold'),bd=4, bg="powder blue",command=lambda: added_value.operation("mul")).grid(row=3, column=3, pady=1)
...
btnZero=Button(calc, text="0",width=6, height=2, font=('arial',20,'bold'),bd=4, bg="powder blue",command=lambda: added_value.numberEnter(0)).grid(row=5, column=1, pady=1)
btnDot=Button(calc, text=".",width=6, height=2, font=('arial',20,'bold'),bd=4, bg="powder blue",command=lambda: added_value.numberEnter(".")).grid(row=5, column=0, pady=1)
...
def iExit():
    iExit=tkinter.messagebox.askyesno("Scientific Calc","Confirm if you want to exit")
    if(iExit>0):
        root.destroy()
        return
